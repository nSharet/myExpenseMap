...
export function buildPieSlices<T>(items: T[], getValue: (item: T) => number): PieSlice<T>[] {
  ...
  return items.map((item, index) => {
    ...
    cursor = endAngle;
    return { item, value, percentage, startAngle, endAngle, path: createDonutPath(startAngle, endAngle) };
  });
}
