...
function TrendChart({ buckets, color, language, formatAmount, formatInteger, active, show, toggle, scheduleHide }: { buckets: TrendBucket[]; color: string; language: "he"|"en"; formatAmount:(n:number)=>string; formatInteger:(n:number)=>string; active:number|null; show:(n:number)=>void; toggle:(n:number)=>void; scheduleHide:()=>void }) {
  ...
  return <div className="trend-chart-wrap"><svg className="trend-chart" viewBox={`0 0 ${W} ${H}`} role="img" aria-labelledby="trend-chart-title trend-chart-desc">
    ...
    {active!==null && buckets[active]?.amount!==null && <foreignObject x={Math.max(L,Math.min(W-R-160,x(active)-80))} y={Math.max(3,y(buckets[active].amount!)-72)} width="160" height="62" onMouseEnter={()=>show(active)} onMouseLeave={scheduleHide}><div className="trend-tooltip" role="tooltip"><b>{periodLabel(buckets[active],language)}</b><strong>{formatAmount(buckets[active].amount!)}</strong><small>{formatInteger(buckets[active].count)} {copy.transactions}{buckets[active].credits<0?` · ${formatAmount(buckets[active].credits)} ${copy.credits}`:""}</small></div></foreignObject>}
  </svg></div>;
}
